...
function Note() {
  ...
  return (
    <div>
      ...
      <li><span>Computers need to predict using a model given new set of inputs. Practically, the model has to deal with independent different data inputs. Testing is a simulation for that assumption. It simulate the outside-of-the-box situation where machines feed newly observed data inputs and predict based on that.</span></li>
      <li><span>If a model constructed training gives poor performance on testing samples, it indicates that the model is not generalised enough; which is called overfitting. It&s like a boxer who performs greatly in training room but very poorly in street fight. A good boxer can perform greatly in street fight too. (one&s skill is generalized enough to fit in new situations.)</span></li>
      ...
      <ul>
      <li><span>Classification has classes, which represents labeled outputs under predefined criterion. ie. animals classified to mammals, reptiles, birds, amphibians, and fish (classes of animals). </span></li>
      ...
      <ul>
      <li><span>ie. Cancer Detection: we want to minimize False Negatives. We should never say you are fine (negative predicted = no cancer predicted) to the patients who actually developing cancers in their body (positive actually = cancer developing). We should say you need further diagnosis even if it turns out to be actually no cancers found at the end (False Positives)..</span></li>
      </ul>
      </ul>
      </ul>
      </ul>
      <li><span>Precision and Recall: confusion matrix helps us to identify each buckets of classifier model&s predictions and actuals. Precision and Recall numericalize a rate of errors for the two error cases. These are more specific versions of accuracy based on types of errors.</span></li>
      <ul>
      <li><span>Precision: how many false alarms we made. The more false positives, we have less precision. The more falses we believe, the less we are precise. = True Positive / (True Positive + False Positive)</span></li>
      <li><span>Recall: how many false ignorances we made. The more false negatives, we have less recall. The more truths we ignore, the worse we recall. = True Positives / (True Positive + False Negative)</span></li>
      </ul>
      ...
      <li><span>Curse of Dimensionality: As the number of features or dimensions grows, the amount of data we need to generalize accurately grows exponentially.</span></li>
      ...
      <li><span>A model tuning should aim at both the testing and training curves converge at similar values and the gap between the training and testing sets, is quite small on learning curve plots.</span></li>
      </ul>
